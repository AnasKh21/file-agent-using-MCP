...
# MCPClient manages the full interaction cycle between this client, the MCP server, and Claude.
# It handles connection setup, tool discovery, and user-facing conversation flows.
class MCPClient:
    ...
    # Initialize session resources and the Anthropic client
    def __init__(self):
    ...
    # Connect to the MCP server process through a supported script transport
    ...
    # Handle server requests for user-provided elicitation data
    ...
    # Display progress updates from the MCP server during long-running operations
    ...
    # Process arbitrary messages and notifications sent by the MCP server
    ...
    # Retrieve the current tool definitions exposed by the MCP server
    ...
    # Retrieve prompt templates available from the MCP server
    ...
    # Retrieve resource definitions exposed by the MCP server
    ...
    # Retrieve templates for resources that the MCP server can use
    ...
    # Send a query to Claude, allowing it to call MCP tools until it returns a final answer
    ...
    # Enter an interactive multi-turn conversation loop with Claude
    ...
    # Execute a named prompt template that is defined on the MCP server
    ...
    # Read a file's contents through the MCP file resource interface
    ...
    # Format and print directory listing data returned by the MCP server
    ...
    # List the current directory contents using the MCP directory resource
    ...
    # Display the interactive menu and route user actions to the correct handlers
    async def menu(self):
...
> """).strip()
    ...
    # Return a quit signal so the menu loop can stop cleanly
    ...
    # Clean up async resources and close any active MCP connections

...
# Convert a relative project path into a safe absolute path inside BASE_DIR
def get_path(relative_path: str) -> Path:
    """Convert relative path to an absolute path within the project directory.

    Ensures the resolved path is inside BASE_DIR for security, then returns
    the absolute path object for safe file and directory operations.
    ...
    if not resolved.exists():
        # If the path does not exist yet, still allow it if parent is safe.
        resolved = resolved
    if BASE_DIR not in resolved.parents and resolved != BASE_DIR:
        raise ValueError(f"{relative_path} is not in the subpath of {BASE_DIR}")
    return resolved
...
#This tool allows the agent to create a new file with specified content. It ensures that parent directories are created if they don't exist, and writes the content to the file using UTF-8 encoding. The tool also reports progress during the writing process and logs success or error messages to the context.
#-----------------------------------------------------------------------------------

# MCP tool: write a file to disk in the project workspace
@mcp.tool()
...
#This tool allows the agent to delete a file from the project directory. It checks if the specified path is a file before attempting deletion, and logs appropriate messages based on the outcome of the operation.
#-----------------------------------------------------------------------------------

# MCP tool: delete a file from the project workspace after validation
@mcp.tool()
...
# MCP resource: read a file's content and expose it through the MCP resource API
...
# MCP resource: list the current directory contents in a structured response
@mcp.resource("dir://.")
...
#This prompt template generates a structured prompt for code review and quality evaluation. It reads the content of a specified code file, detects the programming language based on the file extension, and formats a prompt that includes the file name, language, and current code. The prompt is designed to elicit a comprehensive evaluation of the code from an expert code editor perspective.

# MCP prompt: generate a code review prompt using the contents of a source file
@mcp.prompt()
...
# MCP prompt: generate a documentation task prompt based on file content and user input
@mcp.prompt()
